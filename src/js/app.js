App = {
  ...
  markAdopted: function() {
    ...
    App.contracts.adoption.deployed().then((instance) => {
      ...
      // the smart contract has  function called getAdopters
    }).then((adopters) => {
      for (let i = 0; i < adopters.length; i++) {
        
        if(!web3.toBigNumber(adopters[i]).isZero()) {
          // loop via all the pets 
  ...
  handleAdopt: function(event) {
    ...
    web3.eth.getAccounts( (err,accounts) => {
      ...
      // this is an async call that returens a give instance of 
      // a smart contract
      App.contracts.adoption.deployed().then((instance) => {

        //send a transation with the value of petID, from the 
        // first account  a user has
